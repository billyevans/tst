...
impl<Value: Clone> TST<Value> {
	...
	pub fn put(&mut self, key: &str, val: &Value) {
		unsafe { self.root = self.put_(self.root, key.chars().collect(), val, 0); }
	}

	unsafe fn put_(&mut self, mut x: *mut Node<Value>, key: Vec<char>, val: &Value, d: usize) -> *mut Node<Value> {
		let c = key[d];
		if x.is_null() {
			x = mem::transmute(Box::new(Node::<Value>::new()));
			(*x).c = c;
		}
		if c < (*x).c {
			...
			(*x).val   = mem::transmute(Box::new(val.clone()));
		}
		return x;
	}
